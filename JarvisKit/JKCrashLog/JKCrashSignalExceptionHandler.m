...
@implementation JKCrashSignalExceptionHandler
...
+ (void)signalRegister
{
    JKSignalRegister(SIGABRT);
    JKSignalRegister(SIGBUS);
    JKSignalRegister(SIGFPE);
    JKSignalRegister(SIGILL);
    JKSignalRegister(SIGPIPE);
    JKSignalRegister(SIGSEGV);
    JKSignalRegister(SIGSYS);
    JKSignalRegister(SIGTRAP);
}

#pragma mark Register Signal

static void JKSignalRegister(int signal) {
    struct sigaction action;
    action.sa_sigaction = JKSignalHandler;
...
static void JKSignalHandler(int signal, siginfo_t* info, void* context) {
    ...
    // 保存崩溃日志到沙盒cache目录
    [JKCrashLogHelper jk_saveCrashLog:[NSString stringWithString:mstr] fileName:@"Crash_Signal"];
    
    JKClearSignalRigister();
...
static void JKClearSignalRigister() {
