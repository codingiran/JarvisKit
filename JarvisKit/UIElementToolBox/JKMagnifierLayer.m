...
@interface JKMagnifierLayer ()

@property (nonatomic) struct CGPath *gridCirclePath;

@end

@implementation JKMagnifierLayer
...
- (void)drawGridInContext:(CGContextRef)ctx
{
    ...
    for (j = 0; j < kGridNum; j ++) {
        for (i = 0; i < kGridNum; i ++) {
            ...
            if (self.pointColorHandler) {
                NSString *pointColorHexString = self.pointColorHandler(currentPoint);
                gridColor = [UIColor jk_colorWithHexString:pointColorHexString];
            }
